(ns marketentry.marketentryllm
  ...
  the Republic of Niger (NER) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real marchespublics.ne tender
  response / ARCOP filing. Every output is censored downstream by
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world marchespublics.ne tender
  response / ARCOP filing submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
     ...
     :rationale  (if e
                   (str "nif-verified?=" (:nif-verified? e)
                        " prior-redevance-sanction?=" (:prior-redevance-sanction? e)
                        " redevance-sanction-amount=" (:redevance-sanction-amount e)
                        " redevance-sanction-exclusion-years=" (:redevance-sanction-exclusion-years e)
