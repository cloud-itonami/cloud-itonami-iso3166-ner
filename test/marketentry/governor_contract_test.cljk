  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Niamey Négoce SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Niamey Négoce SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest redevance-sanction-range-violated-is-held-and-unoverridable
  (testing "declared redevance-non-payment sanction amount (500,000 FCFA) falls below Décret 2016-642/PRN/PM Art.7's 1,000,000 FCFA floor -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:redevance-sanction-range-violated} (-> (store/ledger db) last :basis)))
...
(deftest nif-record-unverified-is-held-and-unoverridable
  (testing "unverified NIF registration when required -> HARD hold"
    (let [[db actor] (fresh)
...
(deftest redevance-sanction-within-range-is-not-gated-by-the-flagship-check
  (testing "eng-6 declares a prior redevance-non-payment sanction that IS within Décret 2016-642/PRN/PM Art.7's bounds (5,000,000 FCFA / 2-year exclusion) -- the flagship check never fires for it"
    (let [[_db actor] (fresh)
